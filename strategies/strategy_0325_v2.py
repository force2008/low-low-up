...
class Strategy:
    ...
    def check_60m_divergence(self, df_60m: List[tuple], idx: int) -> Tuple[bool, str, float, float]:
        """
        检查 60 分钟底背离条件
        条件：当前绿柱堆 K 线低点 >= 前一个已完成的绿柱堆 K 线低点（允许持平）
    ...
    def check_60m_entry(self, df_60m: List[tuple], idx: int, 
        ...
        return True, f"60m 转红 + {diver_reason}", last_green_low, prev_prev_green_low
    ...
    def check_5m_entry(self, df_5m: List[tuple], idx: int,
        ...
        """
        检查 5 分钟入场条件（与 backtest_v7.py 一致）
        - MACD 红柱：当前 hist > 0（已注释）
        - 阳柱确认：close > open
        """
        row = df_5m[idx]
        hist_0 = row[8]
        ...
        # 条件 1: MACD 红柱（已注释，与 backtest_v7.py 一致）
        # if hist_0 <= 0:
...
class LiveConfig:
    ...
    MAX_5M_BARS = 5000  # 最多保留 5000 根 5 分钟 K 线（与 backtest_v7.py 一致）
...
class TrendReversalStrategy:
    ...
    def check_60m_dif_turn_in_green(self, df_60m: List[tuple], idx: int,
        ...
        # DIF 一次拐头（与 backtest_v7.py 保持一致）
        if dif_3 > dif_2 > dif_1 < dif_0:
            return True, "60m 绿柱堆内 DIF 拐头"
...
class LiveStrategyEngine:
    ...
    def _check_strategy_on_60m_complete_at_index(self, idx_60m: int):
        """在指定60分钟索引处检查策略条件（批量回测专用）"""
        if len(self.df_60m_with_macd) < 5:
            return
    ...
    def _check_5m_entry(self):
        ...
        if all_signals:
            ...
            for signal in (self.precheck_signals_green + self.precheck_signals_red)[:]:
                ...
                # 绿柱堆和红柱堆内信号都需要检查底背离（与 backtest_v7.py 一致）
                ...
                if not diver_ok:
                    ...
                    if signal_type == 'green':
                        pass  # diver_reason already set
                    else:
                        diver_reason = f"红柱堆内 DIF 拐头 + {diver_reason}"
    ...
    def _check_5m_entry_at_index(self, idx_5m: int):
        """在指定索引处检查5分钟入场条件（批量回测专用）"""
        if self.position is not None:
            return  # 已有持仓
        ...
        # 检查冷却期
        if self.last_entry_time:
            current_dt = datetime.strptime(self.df_5m_with_macd[idx_5m][0][:19], '%Y-%m-%d %H:%M:%S')
            hours_passed = (current_dt - self.last_entry_time).total_seconds() / 3600
            if hours_passed < self.config.COOLDOWN_HOURS:
                return
        
        current_time = self.df_5m_with_macd[idx_5m][0][:19]
        idx_60m = len(self.df_60m_with_macd) - 1  # 使用最新的60分钟数据
            ...
            if dif_turn_red:
                ...
                if cond_5m:
                    ...
                    if initial_stop_loss is not None:
                        entry_price = self.df_5m_with_macd[idx_5m][4]
                        self._create_entry_signal(entry_price, initial_stop_loss, stop_reason,
                                               f"60m 红柱 DIF 拐头 + {reason_5m}", current_time)

    def _check_stop_loss_at_index(self, idx_5m: int):
        """在指定索引处检查止损（批量回测专用）"""
        if self.position is None:
            return
        
        current_bar = self.df_5m_with_macd[idx_5m]
        current_low = current_bar[2]
        current_time = current_bar[0]
    ...
    def run_batch_backtest(self, df_5m: List[tuple], df_60m: List[tuple]) -> List[Signal]:
        """
        批量回测模式 - 与 backtest_0325_v2.py 保持完全一致的逻辑
        """
        if not df_5m or not df_60m:
            return []
        ...
        # 初始化策略（使用与backtest相同的逻辑）
        symbol_info = {'PriceTick': 0.2, 'VolumeMultiple': 10}  # 默认值
        strategy = Strategy(symbol_info)

        signals = []

        # 预检查信号队列
        precheck_signals_green = []  # 绿柱堆内 DIF 拐头信号
        precheck_signals_red = []    # 红柱堆内 DIF 拐头信号

        # 冷却期跟踪（与 backtest_0325_v2.py 一致）
        ...
        for i, row_5m in enumerate(df_5m):
            ...
            if idx_60m is None or idx_60m >= len(df_60m):
                continue
            
            # 【优化 1】预检查 60 分钟 DIF 拐头信号
            # 绿柱堆内 DIF 拐头
            dif_turn_green, _ = strategy.check_60m_dif_turn_in_green(df_60m, idx_60m, green_stacks_60m)
            if dif_turn_green:
                # 检查底背离条件
            ...
            # 红柱堆内 DIF 拐头
            hist_0 = df_60m[idx_60m][8]
            if hist_0 > 0:  # 红柱堆内
                dif_4 = df_60m[idx_60m-4][6] if idx_60m >= 4 else 0
                dif_3 = df_60m[idx_60m-3][6] if idx_60m >= 3 else 0
                dif_2 = df_60m[idx_60m-2][6] if idx_60m >= 2 else 0
                dif_1 = df_60m[idx_60m-1][6] if idx_60m >= 1 else 0
                dif_0 = df_60m[idx_60m][6]
                
                dif_turn_red = dif_2 > dif_1 < dif_0
                if dif_turn_red:
            ...
            if all_signals:
                ...
                for signal in (precheck_signals_green + precheck_signals_red)[:]:
                    ...
                    if not diver_ok:
                        ...
                        if signal_type == 'green':
                            diver_reason = f"60m 底背离确认 (低:{curr_low_60m:.2f}→{prev_prev_low_60m:.2f})"
                        else:
                            diver_reason = f"红柱堆内 DIF 拐头 + {diver_reason}"
                    ...
                    if cond_5m:
                        ...
                        # 计算仓位大小
                        volume_multiple = symbol_info.get('VolumeMultiple', 10)
                        contract_value = entry_price * volume_multiple
                        ...
                        new_signal = Signal(
                            ...
                            extra_data={'source': signal_type}
                        )
                        
                        print(f"DEBUG: 创建信号 - 时间:{row_5m[0]} 价格:{entry_price:.2f} 止损:{initial_stop_loss:.2f} 类型:{signal_type}")
                        signals.append(new_signal)

                        # 更新冷却期时间（与 backtest_v7.py 一致）
...
def main():
    """主函数 - 从 main_contracts.json 批量回测并输出信号到 CSV"""
    print("="*80)
    print("main_contracts 批量回测（5分钟K线、1000条、信号导出CSV）")
    print("="*80)

    db_path = LiveConfig.DB_PATH
    contracts_path = LiveConfig.CONTRACTS_PATH
    max_bars = LiveConfig.MAX_5M_BARS  # 使用配置中的最大K线数量
    output_file = "backtest_signals.csv"

    with open(contracts_path, 'r', encoding='utf-8') as f:
        contracts = json.load(f)

    active_contracts = [c for c in contracts if c.get('IsTrading', 0) == 1]
    # active_contracts = active_contracts[:3]  # 暂时只测试前3个合约
    print(f"共加载 {len(active_contracts)} 个活跃合约")
    # 输出 CSV 表头
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=[
            'symbol', 'time', 'signal_type', 'price', 'stop_loss', 'position_size', 'reason', 'source'
        ])
        writer.writeheader()

        for idx, contract in enumerate(active_contracts, start=1):
            main_symbol = contract.get('MainContractID')
            exch = contract.get('ExchangeID', '').strip()
            symbol = f"{exch}.{main_symbol}" if exch else main_symbol

            print(f"[{idx}/{len(active_contracts)}] 回测 {symbol} ...")

            config = LiveConfig()
            config.TARGET_NOTIONAL = 100000  # 增加目标货值
            engine = LiveStrategyEngine(symbol, db_path=db_path, contracts_path=contracts_path, config=config)

            # 使用历史数据（不直接调用 initialize，避免历史K线与on_5m_bar重复）
            df_5m = engine.data_loader.load_kline_fast(symbol, engine.config.DURATION_5M, max_bars)
            df_60m = engine.data_loader.load_kline_fast(symbol, engine.config.DURATION_60M)

            print(f"  数据加载: 5m={len(df_5m)} 根, 60m={len(df_60m)} 根")

            if len(df_5m) < 50 or len(df_60m) < 20:
                print(f"  跳过 {symbol}：数据不足 (5m={len(df_5m)}, 60m={len(df_60m)})")
                engine.data_loader.close()
                continue

            # 初始化 60m 数据与状态
            engine.df_60m = df_60m
            engine.df_60m_with_macd, engine.green_stacks_60m, engine.green_gaps_60m = StackIdentifier.identify(
                MACDCalculator.calculate(engine.df_60m)
            )
            engine.last_60m_bar_time = engine.df_60m_with_macd[-1][0] if engine.df_60m_with_macd else None

            # 使用批量回测模式（大幅提升性能）
            signals = engine.run_batch_backtest(df_5m, df_60m)
    ...
    try:
        ...
        with open(output_file, 'r', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                symbol = row['symbol']
                if symbol not in signals_by_contract:
                    signals_by_contract[symbol] = []
                signals_by_contract[symbol].append(row)
                total_signals += 1
        
        contracts_with_signals = len(signals_by_contract)
        
        # 基于信号数据生成交易记录（参考backtest文件的逻辑）
        all_trades = []
        
        for symbol, signals in signals_by_contract.items():
            # 按时间排序信号
            ...
            for i, signal in enumerate(sorted_signals):
                ...
                entry_conditions = signal['reason']
                
                # 计算平仓信息
                if i < len(sorted_signals) - 1:
                    # 用下一个信号作为平仓
                    next_signal = sorted_signals[i + 1]
                    exit_time = next_signal['time']
                    exit_price = float(next_signal['price'])
                    exit_reason = "next_signal"
                else:
                    # 最后一个信号，假设小幅盈利平仓
                    exit_time = entry_time  # 简化处理
        ...
        print("📊 批量回测统计报告")
        print("="*60)
        print(f"   总合约数：{len(active_contracts)}")
