#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
TrendReversalV4Strategy - 趋势反转策略 V4
优化：基于 MACD 红柱堆的低点比较
- 找到最近的 MACD 红柱堆（连续红柱）
- 如果红柱数量 ≤ 2 根，继续往前找前一个红柱堆
- 比较两个红柱堆期间的最低点
"""

import numpy as np
from typing import List, Dict, Optional, Tuple
from dataclasses import dataclass

@dataclass
class Contract:
    ProductID: str
    MainContractID: str
    VolumeMultiple: int
    PriceTick: float

@dataclass
class Signal:
    symbol: str
    entry_time: str
    entry_price: float
    exit_time: str
    exit_price: float
    exit_reason: str
    pnl_pct: float
    pnl_amount: float
    pnl_per_hand: float
    entry_idx: int
    stop_loss: float
    take_profit: float

class TrendReversalV4Strategy:
    """趋势反转策略 V4 - MACD 红柱堆优化版"""
    
    def __init__(self, config: Dict = None):
        self.config = {
            'rsi_period': 14,
            'vol_multiplier': 1.3,
            'breakout_pct': 0.005,
            'macd_fast': 12,
            'macd_slow': 26,
            'macd_signal': 9,
            'stop_loss_pct': 0.03,
            'take_profit_pct': 0.05,
            'cooldown_bars': 30,
            'max_holding_bars': 100,
            # V4 新增：MACD 红柱堆参数
            'min_red_bars': 3,  # 最少红柱数量
            'max_lookback_bars': 100,  # 最大回溯 K 线数
        }
        
        if config:
            self.config.update(config)
        
        self.contracts = {}
    
    def load_contracts(self, contracts_path: str):
        import json
        with open(contracts_path, 'r', encoding='utf-8') as f:
            contracts_list = json.load(f)
        self.contracts = {c['MainContractID']: Contract(**{
            k: v for k, v in c.items() 
            if k in ['ProductID', 'MainContractID', 'VolumeMultiple', 'PriceTick']
        }) for c in contracts_list}
    
    def _calc_macd(self, data: List[Dict]) -> Tuple:
        """计算 MACD (含柱状图)"""
        closes = [d['close'] for d in data]
        n = len(closes)
        
        # EMA12 和 EMA26
        ema12 = np.zeros(n)
        ema26 = np.zeros(n)
        
        ema12[0] = closes[0]
        ema26[0] = closes[0]
        
        for i in range(1, n):
            ema12[i] = (closes[i] - ema12[i-1]) * 2/13 + ema12[i-1]
            ema26[i] = (closes[i] - ema26[i-1]) * 2/27 + ema26[i-1]
        
        macd_line = ema12 - ema26
        
        # Signal line
        sig_line = np.zeros(n)
        sig_line[0] = macd_line[0]
        for i in range(1, n):
            sig_line[i] = (macd_line[i] - sig_line[i-1]) * 2/10 + sig_line[i-1]
        
        # Histogram (MACD 柱状图)
        histogram = macd_line - sig_line
        
        return macd_line, sig_line, histogram
    
    def _find_red_bar_stacks(self, histogram: np.ndarray, current_idx: int, 
                              min_bars: int = 3, max_lookback: int = 100) -> List[Dict]:
        """
        从当前位置往前找 MACD 红柱堆
        
        Args:
            histogram: MACD 柱状图数组
            current_idx: 当前位置
            min_bars: 最少红柱数量
            max_lookback: 最大回溯 K 线数
            
        Returns:
            红柱堆列表，每个红柱堆包含：
            - start_idx: 起始索引
            - end_idx: 结束索引
            - bars: 红柱数量
            - low_price: 期间最低价
            - low_idx: 最低价索引
        """
        red_stacks = []
        start_idx = max(0, current_idx - max_lookback)
        
        i = current_idx
        while i >= start_idx:
            # 跳过绿柱
            if histogram[i] >= 0:
                i -= 1
                continue
            
            # 找到红柱堆的结束位置
            stack_end = i
            
            # 往前找连续红柱
            stack_start = i
            while stack_start >= start_idx and histogram[stack_start] < 0:
                stack_start -= 1
            stack_start += 1  # 回到第一个红柱
            
            stack_bars = stack_end - stack_start + 1
            
            # 如果红柱数量 >= min_bars，记录这个红柱堆
            if stack_bars >= min_bars:
                # 找红柱堆期间的最低价
                # 注意：这里用 K 线的 low，不是 MACD 柱状图的值
                red_stacks.append({
                    'start_idx': stack_start,
                    'end_idx': stack_end,
                    'bars': stack_bars,
                    'low_idx': -1,  # 待填充
                    'low_price': 0.0  # 待填充
                })
            
            # 跳到红柱堆前面继续找
            i = stack_start - 1
        
        return red_stacks
    
    def _check_low_rising_v4(self, data_60: List[Dict], histogram: np.ndarray, 
                              current_idx: int) -> Tuple[bool, str, Optional[float], Optional[float]]:
        """
        V4 优化版低点抬高判断 - 基于 MACD 红柱堆的低点比较
        
        Returns:
            (is_rising, reason, current_low, prev_low)
        """
        if len(data_60) < 60:
            return False, "数据不足", None, None
        
        # 找红柱堆
        min_bars = self.config.get('min_red_bars', 3)
        max_lookback = self.config.get('max_lookback_bars', 100)
        
        red_stacks = self._find_red_bar_stacks(histogram, current_idx, min_bars, max_lookback)
        
        if len(red_stacks) < 2:
            # 红柱堆不足 2 个，无法比较
            # 退回到传统方法
            recent_lows = [d['low'] for d in data_60[max(0,current_idx-9):current_idx+1]]
            prev_lows = [d['low'] for d in data_60[max(0,current_idx-19):max(1,current_idx-9)]]
            
            if len(recent_lows) > 0 and len(prev_lows) > 0:
                recent_low = min(recent_lows)
                prev_low = min(prev_lows)
                is_rising = recent_low > prev_low * 0.98
                return is_rising, f"红柱堆不足 (仅{len(red_stacks)}个), 使用传统方法", recent_low, prev_low
            else:
                return False, "数据不足", None, None
        
        # 获取最近两个红柱堆
        current_stack = red_stacks[0]  # 最近的红柱堆
        prev_stack = red_stacks[1]     # 前一个红柱堆
        
        # 填充红柱堆的最低价
        lows = np.array([d['low'] for d in data_60])
        
        current_low_idx = np.argmin(lows[current_stack['start_idx']:current_stack['end_idx']+1])
        current_stack['low_idx'] = current_stack['start_idx'] + current_low_idx
        current_stack['low_price'] = lows[current_stack['low_idx']]
        
        prev_low_idx = np.argmin(lows[prev_stack['start_idx']:prev_stack['end_idx']+1])
        prev_stack['low_idx'] = prev_stack['start_idx'] + prev_low_idx
        prev_stack['low_price'] = lows[prev_stack['low_idx']]
        
        current_low = current_stack['low_price']
        prev_low = prev_stack['low_price']
        
        # 判断低点是否抬高
        threshold = prev_low * 0.98
        is_rising = current_low > threshold
        
        reason = (f"红柱堆低点比较：当前红柱堆 ({current_stack['bars']}根) 最低{current_low:.2f} "
                 f"> 前红柱堆 ({prev_stack['bars']}根) 最低{prev_low:.2f} × 0.98 = {threshold:.2f}")
        
        return is_rising, reason, current_low, prev_low
    
    def _check_oversold_v4(self, data_60: List[Dict], histogram: np.ndarray, 
                           current_idx: int) -> Tuple[bool, str, Optional[float]]:
        """
        V4 优化版超跌判断 - 基于 MACD 红柱堆
        
        Returns:
            (is_oversold, reason, pullback_depth)
        """
        if len(data_60) < 60:
            return False, "数据不足", None
        
        closes = np.array([d['close'] for d in data_60])
        current_price = closes[current_idx]
        
        # 找红柱堆
        min_bars = self.config.get('min_red_bars', 3)
        max_lookback = self.config.get('max_lookback_bars', 100)
        
        red_stacks = self._find_red_bar_stacks(histogram, current_idx, min_bars, max_lookback)
        
        reasons = []
        pullback = 0
        
        # 1. 从最近红柱堆的高点回调
        if len(red_stacks) >= 1:
            current_stack = red_stacks[0]
            stack_high = np.max([data_60[i]['high'] for i in range(
                current_stack['start_idx'], current_stack['end_idx']+1)])
            
            pullback_from_stack = (stack_high - current_price) / stack_high
            if pullback_from_stack > 0.02:  # 回调 2%
                reasons.append(f"红柱堆高点回调{pullback_from_stack*100:.1f}%")
                pullback = max(pullback, pullback_from_stack)
        
        # 2. 传统 30K 线高点回调 (备选)
        recent_high = np.max([d['high'] for d in data_60[max(0,current_idx-29):current_idx+1]])
        pullback_traditional = (recent_high - current_price) / recent_high
        
        if pullback_traditional > 0.03:
            reasons.append(f"30K 线高点回调{pullback_traditional*100:.1f}%")
            pullback = max(pullback, pullback_traditional)
        
        is_oversold = pullback > 0.02
        reason = " | ".join(reasons) if reasons else f"回调不足{pullback*100:.1f}%"
        
        return is_oversold, reason, pullback
    
    def _check_macd_divergence_v4(self, data_5: List[Dict], histogram: np.ndarray, 
                                   current_idx: int) -> Tuple[bool, str]:
        """
        V4 优化版 MACD 背离判断
        
        Returns:
            (is_divergence, reason)
        """
        if len(data_5) < 60:
            return False, "数据不足"
        
        macd_line, sig_line, hist = self._calc_macd(data_5)
        
        # 1. MACD 上升 (当前值 > 5 根 K 线前)
        macd_rising = macd_line[current_idx] > macd_line[current_idx - 5] if current_idx >= 5 else False
        
        # 2. 柱状图由红转绿 (动能转正)
        hist_current = hist[current_idx]
        hist_prev = hist[current_idx - 3] if current_idx >= 3 else hist_current
        hist_turning_green = hist_current > 0 and hist_prev < 0
        
        # 3. 柱状图低点抬高 (底背离)
        if current_idx >= 10:
            hist_recent_min = np.min(hist[current_idx-9:current_idx+1])
            hist_prev_min = np.min(hist[max(0,current_idx-19):max(1,current_idx-9)])
            hist_divergence = hist_recent_min > hist_prev_min
        else:
            hist_divergence = False
        
        # 4. 价格创新低但 MACD 未创新低 (经典底背离)
        closes = np.array([d['close'] for d in data_5])
        if current_idx >= 20:
            price_low_recent = np.min(closes[current_idx-9:current_idx+1])
            price_low_prev = np.min(closes[max(0,current_idx-19):max(1,current_idx-9)])
            price_new_low = price_low_recent < price_low_prev
            macd_not_new_low = macd_line[current_idx] > np.min(macd_line[max(0,current_idx-19):max(1,current_idx-9)])
            classic_divergence = price_new_low and macd_not_new_low
        else:
            classic_divergence = False
        
        reasons = []
        if macd_rising: reasons.append("MACD 上升")
        if hist_turning_green: reasons.append("柱转绿")
        if hist_divergence: reasons.append("柱低点抬高")
        if classic_divergence: reasons.append("底背离")
        
        is_divergence = macd_rising or hist_divergence or classic_divergence
        reason = " | ".join(reasons) if reasons else "无背离信号"
        
        return is_divergence, reason
    
    def _check_trend_filter(self, data_60: List[Dict]) -> Tuple[bool, str]:
        """趋势过滤"""
        if len(data_60) < 60:
            return True, "数据不足"
        
        closes = np.array([d['close'] for d in data_60])
        
        # MA20 和 MA60
        ma20 = np.mean(closes[-20:])
        ma60 = np.mean(closes[-60:])
        
        price_above_ma60 = closes[-1] > ma60
        ma_bullish = ma20 > ma60
        
        # 低点不再创新低
        recent_lows = np.min(closes[-10:])
        prev_lows = np.min(closes[-20:-10])
        low_rising = recent_lows > prev_lows * 0.98
        
        reasons = []
        if price_above_ma60: reasons.append("价格>MA60")
        if ma_bullish: reasons.append("MA20>MA60")
        if low_rising: reasons.append("低点抬高")
        
        is_uptrend = price_above_ma60 or ma_bullish or low_rising
        reason = " | ".join(reasons) if reasons else "下跌趋势"
        
        return is_uptrend, reason
    
    def run_backtest(self, symbol: str, data_5min: List[Dict], data_60min: List[Dict]) -> Optional[List[Signal]]:
        """运行回测 (V4 版本)"""
        if len(data_5min) < 200 or len(data_60min) < 50:
            return None
        
        contract = self._get_contract(symbol)
        if not contract:
            return None
        
        signals = []
        last_signal_idx = 0
        
        # 预先计算 MACD
        macd_60, sig_60, hist_60 = self._calc_macd(data_60min)
        
        print(f"\n{'='*60}")
        print(f"📊 V4 回测：{symbol}")
        print(f"{'='*60}")
        
        for i in range(100, len(data_5min) - 10):
            if i - last_signal_idx < self.config['cooldown_bars']:
                continue
            
            current_5 = data_5min[i-100:i]
            current_time = data_5min[i-1]['time']
            
            data_60_completed = [d for d in data_60min if d['time'] < current_time]
            if len(data_60_completed) < 50:
                continue
            
            # 找到对应的 60 分钟索引
            k60_idx = len(data_60_completed) - 1
            
            # 1. 趋势过滤
            trend_ok, trend_reason = self._check_trend_filter(data_60_completed)
            if not trend_ok:
                continue
            
            # 2. V4 低点抬高 (基于 MACD 红柱堆)
            low_ok, low_reason, current_low, prev_low = self._check_low_rising_v4(
                data_60_completed, hist_60, k60_idx)
            if not low_ok:
                continue
            
            # 3. V4 超跌判断
            oversold_ok, oversold_reason, pullback = self._check_oversold_v4(
                data_60_completed, hist_60, k60_idx)
            if not oversold_ok:
                continue
            
            # 4. V4 MACD 背离
            macd_ok, macd_reason = self._check_macd_divergence_v4(
                current_5, self._calc_macd(current_5)[2], len(current_5)-1)
            if not macd_ok:
                continue
            
            # 生成信号
            entry_price = data_5min[i-1]['close']
            entry_time = data_5min[i-1]['time']
            
            stop_loss = entry_price * (1 - self.config['stop_loss_pct'])
            take_profit = entry_price * (1 + self.config['take_profit_pct'])
            
            signal = self._simulate_trade(symbol, data_5min, i, contract, stop_loss, take_profit)
            if signal:
                signals.append(signal)
                last_signal_idx = i
                
                print(f"✅ {entry_time} @ {entry_price:.2f}")
                print(f"   低点：{low_reason}")
                print(f"   超跌：{oversold_reason}")
                print(f"   MACD: {macd_reason}")
        
        if signals:
            print(f"\n📈 共 {len(signals)} 个信号")
        else:
            print(f"\n⚠️ 无交易信号")
        
        return signals if signals else None
    
    def _simulate_trade(self, symbol: str, data_5min: List[Dict], entry_idx: int, 
                       contract: Contract, stop_loss: float, take_profit: float) -> Optional[Signal]:
        """模拟交易"""
        entry_price = data_5min[entry_idx - 1]['close']
        entry_time = data_5min[entry_idx - 1]['time']
        
        exit_price = None
        exit_time = None
        exit_reason = ""
        max_bars = self.config.get('max_holding_bars', 100)
        
        for j in range(entry_idx, min(entry_idx + max_bars, len(data_5min) - 1)):
            curr_bar = data_5min[j]
            
            if curr_bar['low'] <= stop_loss:
                exit_price = stop_loss
                exit_time = curr_bar['time']
                exit_reason = "止损"
                break
            
            if curr_bar['high'] >= take_profit:
                exit_price = take_profit
                exit_time = curr_bar['time']
                exit_reason = "止盈"
                break
        
        if exit_price is None:
            last = data_5min[min(entry_idx + max_bars, len(data_5min) - 1)]
            exit_price = last['close']
            exit_time = last['time']
            exit_reason = "到期"
        
        position = 1
        total_pnl = (exit_price - entry_price) * contract.VolumeMultiple * position
        pnl_pct = total_pnl / (entry_price * contract.VolumeMultiple * position)
        pnl_per_hand = total_pnl / position
        
        return Signal(
            symbol=symbol,
            entry_time=entry_time,
            entry_price=entry_price,
            exit_time=exit_time,
            exit_price=exit_price,
            exit_reason=exit_reason,
            pnl_pct=pnl_pct,
            pnl_amount=total_pnl,
            pnl_per_hand=pnl_per_hand,
            entry_idx=entry_idx - 1,
            stop_loss=stop_loss,
            take_profit=take_profit
        )
    
    def _get_contract(self, symbol: str) -> Optional[Contract]:
        parts = symbol.split('.')
        if len(parts) != 2:
            return None
        return self.contracts.get(parts[1])


def main():
    import sqlite3
    import sys
    
    db_path = '/home/ubuntu/quant/ctp.examples/openctp-ctp2tts/kline_data.db'
    contracts_path = '/home/ubuntu/quant/ctp.examples/openctp-ctp2tts/main_contracts.json'
    
    print("\n🎯 TrendReversalV4 策略回测 (MACD 红柱堆优化版)\n")
    
    strategy = TrendReversalV4Strategy()
    strategy.load_contracts(contracts_path)
    
    symbol = sys.argv[1] if len(sys.argv) > 1 else "CFFEX.IC2606"
    
    # 加载数据
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    
    cursor.execute("SELECT datetime, open, high, low, close, volume FROM kline_data WHERE symbol = ? AND duration = 300 ORDER BY datetime DESC LIMIT 500", (symbol,))
    data_5min = [{'time': row[0], 'open': row[1], 'high': row[2], 'low': row[3], 'close': row[4], 'volume': row[5]} for row in reversed(cursor.fetchall())]
    
    cursor.execute("SELECT datetime, open, high, low, close, volume FROM kline_data WHERE symbol = ? AND duration = 3600 ORDER BY datetime DESC LIMIT 200", (symbol,))
    data_60min = [{'time': row[0], 'open': row[1], 'high': row[2], 'low': row[3], 'close': row[4], 'volume': row[5]} for row in reversed(cursor.fetchall())]
    
    conn.close()
    
    print(f"5 分钟：{len(data_5min)}条 | 60 分钟：{len(data_60min)}条")
    
    # 运行回测
    signals = strategy.run_backtest(symbol, data_5min, data_60min)
    
    if signals:
        total_pnl = sum(s.pnl_amount for s in signals)
        wins = len([s for s in signals if s.pnl_amount > 0])
        print(f"\n💰 总盈亏：{total_pnl:+,.0f}元 | 胜率：{wins/len(signals)*100:.1f}%")


if __name__ == '__main__':
    main()
