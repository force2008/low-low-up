...
class BatchIndicatorAnalyzer:
    ...
    # 当前绿柱堆最低价 > 前一个绿柱堆最低价 = 抬升
    if current_stack_low > prev_stack_low:
        return True, f"绿柱堆抬升: 当前{current_stack_low:.2f} > 前一个{prev_stack_low:.2f}"

    return False, f"绿柱堆未抬升: 当前{current_stack_low:.2f} <= 前一个{prev_stack_low:.2f}"


    def _check_60m_dif_turn_new(df_60m: List[tuple], idx: int) -> Tuple[bool, str]:
    """检查60分钟DIF是否拐头向上

    DIF拐头：从下跌转为上涨，即DIF从下降变为上升
    """
    if idx < 4:
        return False, "数据不足"
    ...
    def _get_5m_stop_loss(df_5m: List[tuple], idx: int,
                    green_stacks_5m: Dict) -> Tuple[float, str]:
    """获取5分钟的前一个绿柱堆最低价作为止损价

    Args:
        df_5m: 5分钟K线数据
        idx: 当前索引
        green_stacks_5m: 5分钟绿柱堆字典

    Returns:
        (止损价, 原因)
    """
    if not green_stacks_5m:
        return float('inf'), "无绿柱堆信息"
    ...
    for stack_idx in sorted_stacks:
        if stack_idx < idx:
            stack_info = green_stacks_5m[stack_idx]
            return stack_info.get('low', float('inf')), f"前一个绿柱堆最低价"

    return float('inf'), "找不到前一个绿柱堆"


    def _check_5m_entry_condition(df_5m: List[tuple], idx: int,
                           green_stacks_5m: Dict) -> Tuple[bool, str]:
    """检查5分钟入场条件

    5分钟需要满足：在绿柱堆中，或者刚离开绿柱堆
    """
    if idx < 10:
        return False, "数据不足"
    ...
    if hist < 0:
        return True, "在5分钟绿柱堆中"

    # 检查前一根是否在绿柱堆
    if idx > 0 and df_5m[idx-1][8] < 0:
        return True, "刚离开5分钟绿柱堆"

    # 检查前几根是否在绿柱堆（有回调也可以）
    for lookback in range(1, 5):
        if idx - lookback >= 0 and df_5m[idx - lookback][8] < 0:
            return True, f"5分钟绿柱堆回调中"

    return False, "不符合5分钟入场条件"
