...
# 从拆分后的模块导入（公共类）
from strategy import MACDCalculator, ATRCalculator, StackIdentifier, BOLLCalculator
...
# 默认启用的策略
DEFAULT_STRATEGIES = {
    'low_low_up': StrategyLowLowUp,
    'boll_middle_band': StrategyBollMiddleBand,
}
...
class KlineAggregator:
    ...
    def __init__(self, db_manager, instruments, strategy_signal_manager=None):
        ...
        # 策略名称
        self.strategy_name = StrategyLowLowUp({}).name

        # 多策略注册表（K 线合成仍由 KlineAggregator 负责，信号检查下放到策略）
        self.strategies = {
            'low_low_up': StrategyLowLowUp({}),
            'boll_middle_band': StrategyBollMiddleBand({}),
        }
        # 负责持仓交易的核心策略（保持 low_low_up 不变）
        self.active_trading_strategy = 'low_low_up'
    ...
    def check_60m_all_limits(self, symbol: str, data_60m: list, idx_60m: int) -> bool:
        ...
        strategy = self.strategies['low_low_up']
    ...
    def add_tick(self, instrument_name, price, volume, open_interest, timestamp):
        ...
        # 记录最新价格，用于盘中日线/60分钟检查
        self.last_tick_prices[instrument_name] = price
    ...
    def save_kline(self, instrument_name, kline, duration=300):
        ...
        if duration == 300:
            ...
            # BOLL 中轨策略检查（每 5 分钟），用当前 5 分钟收盘价作为盘中最新价
            instrument_name = symbol.split('.')[-1] if '.' in symbol else symbol
            last_price = self.last_tick_prices.get(instrument_name, kline['close'])
            self.check_boll_middle_band_signal(symbol, last_price=last_price)
        else:
            period_name = self._get_period_name_by_duration(duration)
            # 60 分钟 K 线完成后，检查是否产生预检测信号
            if duration == 3600:
                self.check_60m_precheck(symbol, end_time=date_time_str)
            ...
            for sig in valid_precheck:
                ...
                if sig_type == 'green':
                    ...
                    if stop_loss is None:
                        logger.info(f"[{symbol}] 止损计算失败：{stop_reason}")
                        continue
    ...
    def check_boll_middle_band_signal(self, symbol: str, last_price: float = None, end_time: str = None):
        """检查 BOLL 中轨反弹/回踩信号（每 5 分钟运行一次）

        Args:
            symbol: 合约代码（含交易所前缀）
            last_price: 当前最新价格，用于构造盘中虚拟日K/60分钟K线
            end_time: 截止时间，主要用于测试避免未来数据
        """
        try:
            strategy = self.strategies['boll_middle_band']
            now = datetime.now()

            # 日线数据
            data_day = self.db_manager.get_kline_data(symbol, 500, 86400, end_time)
            if len(data_day) >= 50 and last_price is not None:
                # 构造盘中虚拟日 K 线：用当前最新价作为今天收盘价
                today_bar = (now.strftime('%Y-%m-%d 00:00:00'), last_price, last_price, last_price, last_price, 0)
                data_day = list(data_day) + [today_bar]
            if len(data_day) >= 50:
                data_day_with_boll = BOLLCalculator.calculate(data_day)
                signal = strategy.check_boll_middle_band_signal(
                    data_day_with_boll, timeframe="day", symbol=symbol
                )
                if signal:
                    self._send_boll_signal(symbol, signal)

            # 60 分钟数据
            data_60m = self.db_manager.get_kline_data(symbol, 500, 3600, end_time)
            if len(data_60m) >= 50 and last_price is not None:
                # 构造盘中虚拟 60 分钟 K 线
                bar_minute = (now.minute // 60) * 60
                bar_time = now.replace(minute=bar_minute, second=0, microsecond=0)
                current_bar = (bar_time.strftime('%Y-%m-%d %H:%M:%S'), last_price, last_price, last_price, last_price, 0)
                # 如果最后一根 60m K 线时间已经是当前周期，则替换；否则追加
                if data_60m and data_60m[-1][0] == current_bar[0]:
                    data_60m = list(data_60m[:-1]) + [current_bar]
                else:
                    data_60m = list(data_60m) + [current_bar]
            if len(data_60m) >= 50:
            ...
            print_log(f"✗ {symbol} BOLL中轨信号检查失败：{e}")

    def _send_boll_signal(self, symbol: str, signal: dict):
        """发送 BOLL 中轨信号到飞书"""
        signal_type_map = {
            'BOLL_REBOUND_DAY': 'BOLL_REBOUND_DAY',
            'BOLL_PULLBACK_DAY': 'BOLL_PULLBACK_DAY',
            'BOLL_REBOUND_60MIN': 'BOLL_REBOUND_60MIN',
            'BOLL_PULLBACK_60MIN': 'BOLL_PULLBACK_60MIN',
        }
        signal_type = signal_type_map.get(signal['signal_type'], 'BOLL_MIDDLE_BAND')
        ...
        signal_data = {
            ...
            'stop_loss': 0,
            'position_size': 0,
            'strategy_name': self.strategies['boll_middle_band'].name,
            'reason': signal['reason'],
            'time': signal.get('bar_time', datetime.now().isoformat()),
            'message': signal['message'],
        }

        print_log(f"📊 {symbol} BOLL中轨信号：{signal['message']} | {signal['reason']}")
        ...
        try:
            send_feishu_strategy_signal(symbol, signal_data)
            print_log(f"✓ {symbol} BOLL中轨飞书信号已发送")
        except Exception as e:
            print_log(f"✗ {symbol} BOLL中轨飞书信号发送失败：{e}")
