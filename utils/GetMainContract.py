...
class CTdSpi(CTdSpiBase):
    ...
    def OnRspQryInstrument(self, pInstrument: tdapi.CThostFtdcInstrumentField, pRspInfo: tdapi.CThostFtdcRspInfoField,
        ...
        if bIsLast:
            self.print(f"[OnRspQryInstrument] 收到最后一个响应，共 {len(self.instruments)} 个合约")
...
if __name__ == '__main__':
    ...
    while wait_count < max_wait:
        ...
        if spi._is_last:
            break
        if wait_count % 10 == 0:
            print(f"等待合约查询完成... 已等待 {wait_count} 秒，已收到 {len(spi.instruments)} 个合约")

    if not spi._is_last:
        print(f"[警告] 等待 {max_wait} 秒后仍未收到 bIsLast，已收到 {len(spi.instruments)} 个合约")
    else:
        print(f"[完成] 合约查询完成，共 {len(spi.instruments)} 个合约")
