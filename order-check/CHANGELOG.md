...
- **定时任务脚本**：新增 `order-check/scheduler_setup.bat`
  ...
  - 使用 `C:\ProgramData\miniconda3\envs\python310\python.exe`
  - 以 `online` 环境运行
- **多时段自动退出**：仅 `online` 环境生效
  - 11:30 日盘1收盘自动退出
  - 15:15 日盘2收盘自动退出
  - 02:30 夜盘收盘自动退出
  - 退出前发送飞书通知
...
线程	职责
export_thread	每 20 秒导出 CSV，生成 hold-std.json
sync_thread	持续监控 hold-std.json，发现变化就同步
两个线程独立工作，不会重复同步：

export_thread 只负责导出
sync_thread 只负责同步（由文件变化触发，不需要定时）
...
└── 2. 仓位对比
  ...
  - 有差异时：发送详细的缺额/超额合约列表
  - 无差异时：也发送定期通知，显示标准持仓和实际持仓汇总
  - 让用户确认系统一直在检查，不会漏掉同步信息
- **委托单自动撤单重挂**：`order_ops.py` 中 `_check_and_replace_pending_orders()` 方法：
  - 每 45 秒检查一次所有未成交委托
  - 获取当前市场价格，与委托价格对比
  - **价格变化判断**：使用 `main_contracts.json` 中的 `PriceTick` 作为最小价格变动单位
    - 如果价格变化 <= 1 个 PriceTick，保持原委托不动（避免微小波动导致频繁撤单）
    - 如果价格变化 > 1 个 PriceTick，撤单并用最新价格重新提交
  - 防止因价格变动导致订单无法成交
  - 最多重挂 3 次（`MAX_REPLACE_COUNT = 3`），避免无限循环
  - **重挂告警**：达到最大重挂次数仍未成交时，发送飞书告警通知
    - 包含合约、方向、手数、委托价、当前价等信息
    - 提示"请人工处理"，确保不漏掉需要关注的委托

### Changed
- **串行委托提交**：将 `sync.py` 中的并行委托提交（8线程）改为串行执行
  - 改为与 `PositionManagerUI.py` 一致的模式：每笔委托间隔 0.2 秒
  - 避免 CTP API 高并发导致的订单处理延迟
  - 第一阶段：并行查询所有行情（无等待）
  - 第二阶段：串行提交开仓委托（0.2秒间隔）
  - 第三阶段：串行提交平仓委托（0.2秒间隔）

### Fixed
- **成交回报处理优化**：`base.py` 中 `OnRtnTrade()` 回调
  - 移除同步持仓查询（会阻塞最多10秒）
  - 改为使用 `threading.Timer` 延迟 2 秒后异步更新 hold.json
  - 使用 `_hold_update_scheduled` 标志防止重复调度
  - 让批量成交能快速到达，不再一个一个处理
- **监控线程日志增强**：
  - 添加 `[监控] 监控线程启动，等待首次检查...` 日志
  - 添加 `[监控] 开始第 N 次检查 (间隔 45 秒)` 日志
  - 添加 `[监控] 本次检查完成` 日志
  - 便于排查监控线程是否正常启动
